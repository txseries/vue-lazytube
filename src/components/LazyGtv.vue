<template>
    <VideoWrapper :aspectRatioValue="aspectRatioValue" :maxWidth="maxWidth">
      <Preview
          v-if="!onceLoaded"
          @click="createIframe(isVideoFound, 'gtv')">

        <template v-if="isVideoFound ">
          <img v-if="isCustomThumbnailExist" :src="customThumbnail" alt=""
                @error="$event.target.src=getGtvThumbnail(videoID, thumbnailQuality)"
          >
          <img
              v-else
              :src="getGtvThumbnail(videoID, thumbnailQuality)" alt=""
          >
...
// Importing Mixins
import gtvMixin from '../mixins/gtvMixin'
...
export default {
  ...
  computed: {
    videoID: function () {
      console.log("gtvID="+this.getGtvID(this.src));
      return this.getGtvID(this.src)
    },
